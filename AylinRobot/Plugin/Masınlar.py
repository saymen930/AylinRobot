...
@app.on_message(filters.command("araba"))
async def test_bot(bot: app, m: Message):
    start = time()
    replymsg = await m.reply_text("**❤ Araba fotoğrafı seçiliyor...**")
    end = round(time() - start, 2)
    photo = random.choice(photolist)
    text = f"❤️ **{Config.BOT_USERNAME} Sizin için rastgele bir araba resmi seçtim**"
...
async def test_app(app: app, m: Message):
    ...
    text = f"❤️ **{Config.BOT_USERNAME} Sizin için rastgele bir araba videosu seçtim**"
